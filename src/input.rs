//! winit → Ruffle key/control mapping helpers (mirrors `ruffle/desktop/src/util.rs`).
...
pub fn winit_to_ruffle_text_control(
    ...
    text_control_for(event.logical_key.as_ref(), modifiers)
}

fn text_control_for(key: Key<&str>, modifiers: Modifiers) -> Option<TextControlCode> {
...
pub fn winit_input_to_ruffle_key_descriptor(event: &KeyEvent) -> KeyDescriptor {
    KeyDescriptor {
        physical_key: map_winit_physical_key(event.physical_key),
        logical_key: map_logical_key(event.logical_key.as_ref()),
        key_location: map_key_location(event.location),
    }
}

fn map_winit_physical_key(physical: WinitPhysicalKey) -> PhysicalKey {
    match physical {
        WinitPhysicalKey::Code(key_code) => map_physical_key(key_code),
        WinitPhysicalKey::Unidentified(_) => PhysicalKey::Unknown,
    }
}

fn map_physical_key(key_code: WinitKeyCode) -> PhysicalKey {
    match key_code {
...
fn map_logical_key(key: Key<&str>) -> LogicalKey {
    ...
    match key {
        ...
        Key::Character(ch) => alpha_to_logical(ch),
...
fn map_key_location(location: WinitKeyLocation) -> KeyLocation {
    match location {
...
mod tests {
    use super::*;
    use winit::keyboard::{ModifiersState, NativeKeyCode};

    #[test]
    fn enter_is_a_text_control() {
        assert_eq!(
            text_control_for(Key::Named(NamedKey::Enter), Modifiers::default()),
            Some(TextControlCode::Enter)
        );
    }

    #[test]
    fn ctrl_c_is_copy_but_plain_c_is_not() {
        let ctrl = Modifiers::from(ModifiersState::CONTROL);
        assert_eq!(text_control_for(Key::Character("c"), ctrl), Some(TextControlCode::Copy));
        assert_eq!(text_control_for(Key::Character("c"), Modifiers::default()), None);
    }

    #[test]
    fn backspace_and_delete_have_word_variants() {
        let ctrl = Modifiers::from(ModifiersState::CONTROL);
        assert_eq!(
            text_control_for(Key::Named(NamedKey::Backspace), Modifiers::default()),
            Some(TextControlCode::Backspace)
        );
        assert_eq!(
            text_control_for(Key::Named(NamedKey::Backspace), ctrl),
            Some(TextControlCode::BackspaceWord)
        );
        assert_eq!(
            text_control_for(Key::Named(NamedKey::Delete), ctrl),
            Some(TextControlCode::DeleteWord)
        );
    }

    #[test]
    fn arrow_keys_select_and_move_by_word() {
        let shift = Modifiers::from(ModifiersState::SHIFT);
        let ctrl = Modifiers::from(ModifiersState::CONTROL);
        let ctrl_shift = Modifiers::from(ModifiersState::CONTROL | ModifiersState::SHIFT);
        assert_eq!(
            text_control_for(Key::Named(NamedKey::ArrowLeft), Modifiers::default()),
            Some(TextControlCode::MoveLeft)
        );
        assert_eq!(
            text_control_for(Key::Named(NamedKey::ArrowLeft), shift),
            Some(TextControlCode::SelectLeft)
        );
        assert_eq!(
            text_control_for(Key::Named(NamedKey::ArrowLeft), ctrl),
            Some(TextControlCode::MoveLeftWord)
        );
        assert_eq!(
            text_control_for(Key::Named(NamedKey::ArrowRight), ctrl_shift),
            Some(TextControlCode::SelectRightWord)
        );
    }
    ...
    fn letters_map_to_their_physical_key() {
        assert_eq!(map_physical_key(WinitKeyCode::KeyA), PhysicalKey::KeyA);
        assert_eq!(map_physical_key(WinitKeyCode::Numpad0), PhysicalKey::Numpad0);
        assert_eq!(map_physical_key(WinitKeyCode::KeyZ), PhysicalKey::KeyZ);
        assert_eq!(map_physical_key(WinitKeyCode::AudioVolumeUp), PhysicalKey::Unknown);
        assert_eq!(map_physical_key(WinitKeyCode::Escape), PhysicalKey::Escape);
    }

    #[test]
    fn unidentified_physical_keys_map_to_unknown() {
        assert_eq!(
            map_winit_physical_key(WinitPhysicalKey::Unidentified(NativeKeyCode::Unidentified)),
            PhysicalKey::Unknown
        );
    }

    #[test]
    fn named_keys_map_to_ruffle_logical_keys() {
        assert_eq!(
            map_logical_key(Key::Named(NamedKey::Escape)),
            LogicalKey::Named(RuffleNamedKey::Escape)
        );
        assert_eq!(
        ...
        assert_eq!(
            map_logical_key(Key::Named(NamedKey::ArrowUp)),
            LogicalKey::Named(RuffleNamedKey::ArrowUp)
        );
    }

    #[test]
    fn characters_map_to_their_codepoint() {
        assert_eq!(map_logical_key(Key::Character("a")), LogicalKey::Character('a'));
        assert_eq!(map_logical_key(Key::Character("!")), LogicalKey::Character('!'));
    }

    #[test]
    fn key_locations_are_preserved() {
        assert_eq!(map_key_location(WinitKeyLocation::Standard), KeyLocation::Standard);
        assert_eq!(map_key_location(WinitKeyLocation::Left), KeyLocation::Left);
        assert_eq!(map_key_location(WinitKeyLocation::Right), KeyLocation::Right);
        assert_eq!(map_key_location(WinitKeyLocation::Numpad), KeyLocation::Numpad);
    }
}
